...
#   _ AUTH      : token fijo "root" (modo -dev, sin unseal)
...
#   3. Instalación de Vault previa con SCRIPT: [1_Script_Instalacion_Vault_[Helm]_DESA.sh]
...
#   levantado por el SCRIPT de instalación correspondiente: '1_Script_Instalacion_Vault_[Helm]_DESA.sh' 
#
# USO:
#   $ sh ./3_Script_Configuracion_Vault_[Helm]_DESA.sh --modalidad=install
#   $ sh ./3_Script_Configuracion_Vault_[Helm]_DESA.sh --modalidad=delete
# =============================================================================

set -e

# --- Namespace y pod (modo -dev: token fijo "root", sin TLS) ---
NAMESPACE="vault"
VAULT_POD_NAME="vault-0"
DEV_TOKEN="root"
DEV_VAULT_ADDR="http://127.0.0.1:8200"
...
# -----------------------------------------------------------------------------
# PARÁMETROS — Database Secrets Engine (MariaDB/MySQL)
# Genera credenciales dinámicas para la base de datos
# -----------------------------------------------------------------------------
...
PKI_ROOT_TTL="87600h"
VAULT_ROUTE_URL="http://${NAMESPACE}-ui-${NAMESPACE}.apps.itz-44tgug.infra02-lb.dal12.techzone.ibm.com"
...
validate_environment() {
  echo "===> 1. VALIDANDO POD Y VAULT"
  oc get pod "${VAULT_POD_NAME}" -n "${NAMESPACE}"
  echo ""
  VAULT_STATUS=$(oc exec -n "${NAMESPACE}" "${VAULT_POD_NAME}" -- \
    sh -c "export VAULT_ADDR='${DEV_VAULT_ADDR}'; vault status -format=json 2>/dev/null" || true)
  if echo "${VAULT_STATUS}" | grep -q '"initialized":false'; then
    echo "ERROR: Vault no inicializado. Verifica que el pod esté en modo -dev."
    exit 1
  fi
  echo "    ACTIVO (modo DEV). Continuando..."; echo ""
}

# -----------------------------------------------------------------------------
do_install() {
  echo "===== [INSTALL — INICIO] ====="; echo ""

  validate_environment

  echo "===> 2. AUTH METHOD USERPASS"
  run_vault_cmd_allow_error "vault auth enable userpass"; echo ""
  ...
  run_vault_cmd "vault secrets list"; echo ""
  ...
  run_vault_cmd "vault kv put -mount=${KV_MOUNT} ${SECRET_IBM_02_NAME} \
    ...
    cadena='${SECRET_IBM_02_CADENA}'"; echo ""
  ...
  run_vault_cmd "vault write database/roles/${DB_ROLE_NAME} \
    ...
    max_ttl='${DB_MAX_TTL}'"; echo ""

  echo "===> 6. TRANSIT ENGINE"
  run_vault_cmd "vault write -f transit/keys/${TRANSIT_KEY_NAME}"; echo ""
  ...
  run_vault_cmd "vault write ${PKI_MOUNT}/roles/${PKI_WEB_ROLE} \
    ...
    max_ttl='${PKI_WEB_MAX_TTL}' ttl='${PKI_WEB_TTL}'"; echo ""
...
EOF"
  ...
  run_vault_cmd "vault write auth/userpass/users/${APP_USERNAME} \
    password='${APP_PASSWORD}' token_policies='${APP_TOKEN_POLICIES}'"; echo ""
  ...
  run_vault_cmd "vault read database/creds/${DB_ROLE_NAME}"; echo ""
...
do_delete() {
  ...
  # delete borra versión activa; metadata delete borra historial completo
  echo "===> D6. KV SECRETS"
